...
public class GncTimeSeries : IEnumerable<KeyValuePair<DateTime, decimal>>
{
    ...
    /// <summary>
    ///     Retrieves the value of the series at a specified point in time. See Remarks for info on interpolation modes.</summary>
    /// <remarks>
    ///     <para>
    ///         When the specified key does not exist in the data set, <see cref="Get"/> will interpolate or extrapolate as
    ///         necessary. This can be prevented by using interpolation mode "None", in which case an exception will be thrown
    ///         instead.</para>
    ///     <para>
    ///         If extrapolation is required, this method will always extrapolate to the nearest available value. For example,
    ///         the series "5, 8, 12" would be extrapolated as "5, 5, 5, 8, 12, 12, 12".</para></remarks>
    /// <param name="time"/>
